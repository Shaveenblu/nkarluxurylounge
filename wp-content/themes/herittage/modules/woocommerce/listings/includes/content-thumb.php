...
if( ! function_exists( 'herittage_shop_woo_loop_product_thumbnail' ) ) {

	function herittage_shop_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_style != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.herittage_html_output($product_overlay_bgcolor).'></div>';
				}
				...
				if($product_show_offer_percentage) {
					echo herittage_shop_woo_loop_product_offer_percentage($product);
				}


				// Action to run before product thumb image
				do_action( 'herittage_woo_before_product_thumb_image', $product_id );
				...
				if($display_bg_image) {
					...
					echo '<div class="primary-image"><img src="'.esc_url($primary_image_src).'" alt="'.esc_attr__('Shop Primary Image', 'herittage').'" title="'.esc_attr__('Shop Primary Image', 'herittage').'" /></div>';
				}
				...
				if($secondary_image_src != '') {
					if($display_bg_image) {
						...
						echo '<div class="secondary-image"><img src="'.esc_url($secondary_image_src).'" alt="'.esc_attr__('Shop Secondary Image', 'herittage').'" title="'.esc_attr__('Shop Secondary Image', 'herittage').'" /></div>';
					}
				}
			...
			if($product_thumb_content) {
				...
				herittage_shop_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.herittage_html_output($product_thumb_content_overlay_bgcolor).'>';
					do_action('herittage_woo_loop_product_thumb_content', 'thumb');
					remove_all_actions('herittage_woo_loop_product_thumb_content');
				echo '</div>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'herittage_shop_woo_loop_product_thumbnail', 10 );
