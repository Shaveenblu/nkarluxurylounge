<?php
add_action( 'herittage_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'herittage-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, HERITTAGE_THEME_VERSION, 'all');
    }
}

add_filter( 'herittage_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = herittage_404_page_params();
...
function herittage_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'herittage_404_page_params', $params );
}